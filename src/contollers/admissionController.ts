...
export const offerAdmission = async (req: Request, res: Response) => {
  const { gmailExtension, firstName, lastName, surname, departmentId } = req.body;
  try {
    ...
    const email = generateStudentGmail(firstName,lastName, surname, gmailExtension);
    ...
    res.status(StatusCodes.OK).send({msg:"Admission offered sucessfully ", newStudent, admissionLink})
  } catch (error) {
    res.status(StatusCodes.INTERNAL_SERVER_ERROR).send("there was an error offering admission ")
  }
};
